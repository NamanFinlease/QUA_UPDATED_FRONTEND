...
const EditCam = ({ camData, setIsEditing }) => {
  ...
  const [dateTime, setDateTime] = useState({
    disbursalDate: dayjs(camData.disbursalDate),
    repaymentDate: dayjs(camData.repaymentDate),
  });
  ...
  const calculateDaysDifference = (disbursalDate, nextPayDate) => {
    if (!disbursalDate && !nextPayDate) {
      return 0;
    }
    const startDate = new Date(dayjs(disbursalDate));
    const endDate = new Date(dayjs(nextPayDate));
