...
const SanctionLetterPreview = ({
    ...
    console.log('Annual Percentage', annualPercentage)
    //color theme
    ...
    useEffect(() => {
        if (isSuccess && data) {
            Swal.fire({
                text: "Sanction send to User for E-sign!",
                icon: "success"
            });
            handleClose();
            ...
            <Modal open={preview} onClose={handleClose}>
                ...
                <Box mt={4} textAlign="center">
                    ...
                    <Button
                        ...
                        sx={{
                            backgroundColor: (isLoading || isFetching) ? "#9de39e" : colors.white[100],
                            color: (isLoading || isFetching) ? "#666" : colors.greenAccent[600],
                            cursor: (isLoading || isFetching) ? "not-allowed" : "pointer",
                            borderRadius: (isLoading || isFetching) ? "0px 10px" : "0px 10px",
                            margin:"0px 10px",
                            "&:hover": {
                                backgroundColor: (isLoading || isFetching) ? "#9de39e" : colors.greenAccent[600],
                                color: (isLoading || isFetching) ? "#9de39e" : colors.white[100],
                            },
                        }}
                    >
                        {(isLoading || isFetching) ? <CircularProgress size={20} color="inherit" /> : "Send For e-Sign"}
                    </Button>
                </Box>
                </Box>
            </Modal>
